(function($) {
    ...
    var useLinks = prefs.getBool("useLinks");
    ...
    var JqlQuery = {
        ...
        EpicByLink          : "issue%20in%20linkedIssues(%22{0}%22,%22is%20parent%20task%20of%22)"
    };
    ...
    function renderItemStatus(key){
        var url = baseSearchRequest
        if (useLinks) {
            url += JqlQuery.EpicByLink.format(key);
        } else {
            url += JqlQuery.Epic.format(key);
        }
        ...
        for (var i = 0; i < itemNodes.length; i++) {
            ...
            for (var j = 0; j < childNodes.length ; j++) {
                ...
                switch (childNode.nodeName) {
                    ...
                    case "link":
                        if (useLinks) {
                            epic_key = childNode.firstChild.nodeValue;
                        } else {
                            item.link = childNode.firstChild.nodeValue;
                        }
                        break;
                    ...
                    case "customfields":
                        ...
                        break;
                    case "issuelinks":
                        var inwardLinks =  childNode.getElementsByTagName("inwardlinks");
                        for (var l=0;l<inwardLinks.length;l++) {
                            var inwardLink = inwardLinks.item(l);
                            if (!isElement(inwardLink)) {continue;}
                            epic_key = inwardLink.getElementsByTagName("issuekey")[0].firstChild.nodeValue;
                        }
                        break;
                }
            }
            ...
            if (console) console.log(item.key + " -> " + epic_key);
